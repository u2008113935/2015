...
SELECT 
	...
	,ZON.nCodZona, ZON.cDesZona, ZON.cAbrZona
...
FROM [SIPDIncCreCal] A					
	...
	LEFT JOIN [SIPDIncAsiPon] P 
		...
		AND P.cCodPeriodo = '2017/09'
		AND P.cCodSituac = 'A'								
			--/*
	LEFT join [SIPTClaSubNiv] N
		ON	N.cCodNivel = P.cCodNivel 
		AND N.cCodSubNiv = P.cCodSubNiv 
		AND N.cCodClaSub = P.cCodClaSub 
		AND N.lConEstado = 1 
		AND N.nCodPAPApr = 8
			--*/
WHERE A.cCodPeriodo='2017/09'
	AND ZON.nCodZona = '2'
	--AND A.cCodPerson = 'RRAMIR'		
