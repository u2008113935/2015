	...
	/*
		Se requiere saber cual es el asesor y agencia del listado de clientes adjuntos
	...
	*/
	...
	FROM [HYO00402].SOFCMACHYO_DIARIO_MANIANA.dbo.[KPYMCRECONVEN] CRE (NOLOCK)		
			...
			INNER JOIN [HYO00402].SOFCMACHYO_DIARIO_MANIANA.dbo.[GenTDepartame] DEP1
				ON DEP1.cCodDepart = DC.cCodDepart
			INNER JOIN [HYO00402].SOFCMACHYO_DIARIO_MANIANA.dbo.[GentProvincia] PRO1
				ON pro1.cCodProvin = DC.cCodProvin and pro1.cCodDepart = DC.cCodDepart
			INNER JOIN [HYO00402].SOFCMACHYO_DIARIO_MANIANA.dbo.[GentDistrito] DIS1
				ON dis1.cCodDistri = DC.cCodDistri	 and dis1.cCodProvin = DC.cCodProvin 
					and dis1.cCodDepart = DC.cCodDepart					
			
			INNER JOIN [HYO00402].SOFCMACHYO_DIARIO_MANIANA.dbo.GENTZona Z
		...
		WHERE -- CRE.cEstCreCon in ('F','H')					
			...
			CRE.cCodCtaCre COLLATE SQL_Latin1_General_CP1_CI_AS	
				in (Select rtrim(credito) from DatosN)