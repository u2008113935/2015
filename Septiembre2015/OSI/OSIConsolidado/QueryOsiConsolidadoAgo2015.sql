	...
	CONSOLIDADO AGO 2015													
													
	2. Reporte detallado por cliente de las colocaciones al 31.08.2015, 
	   de los créditos Normal, vencido, Judicial, Cartera atrasada, 
	   Cartera refinanciada y reprogramada teniendo en cuenta la siguiente
	   información:											
											
		N° de Orden, Cuenta, Tipo de Crédito, Cliente, Importe Desembolsado (S/),
		Saldo de Capital (S/.), Asesor Solicitud, Agencia, Código de la Solicitud,
		Fecha de Presentación de la Solicitud, Fecha de Desembolso del Crédito,
		Plazo de atención
	...
	*/
	...
	Select * into #tab01 from (
		...
		Select 						
			...
			,CodigoSolicitud = S.cCodSolCre
			,FechaSolicitud  = left(cast(S.dFecSolCre as date),10)
			,left(cast(CRE.dFecDesCre as date),10) as 'FechaDesembolsoCredito'								
			,PlazoAtencion = 
				datediff (DAY,left(cast(S.dFecSolCre as date),10),left(cast(CRE.dFecDesCre as date),10))
	...
	*/	
	...
	-- Select * into #tab02 from (
		...
		*/
		Select Cant= count(*) from #tab01
		Select Saldo = sum(SaldoCapitalenSoles) from #tab01
