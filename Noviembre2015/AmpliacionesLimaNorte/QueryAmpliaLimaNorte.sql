		...
		*/
		...
		/*
		Se solicita informacion, data de clientes aptos para ampliacion de la zona lima norte: 
		Miraflores, Los Olivos, Huacho, Comas, Barranca, Huaral, San Juan de Miraflores, 
		Villa el Salvador, Surquillo, Carabayllo, San Martin de Porres.

		RPM: #945493060
		incidente de referencia: SAR - N° Inc. 101207
		...
		*/
		...
		SET LANGUAGE spanish;
			...
			DECLARE @nTipCambio MONEY,	@dFecTipCam DATE					
				set @nTipCambio = (
					...
					FROM GENTTipCambio
					WHERE left(cast(dFecTipCam as date),10) ='2016-01-01'				
						)
			----------------------------------------------------------------------------------------			
		...
		Select * into #tab01 from (
			...
			FROM [KPYMCRECONVEN] CRE (NOLOCK)		
					...
					left join CMACHYOCLI_MANIANA.dbo.[CLIMDirecc] DC				
		...
		Select * into #sbs01 from (
				...
				WHERE LEFT(B.Cctacon, 4) 
					...
					AND A.CCODSBS COLLATE SQL_Latin1_General_CP1_CI_AS
	...
	------------------------CURSOR-----------------------------------
		...
		-- Select * from #tab04
			...
			*/
			...
			Select CodigoCliente, NombreCliente,Telefono,DireccionDomicilio,DireccionReferencia,ZonaDireccion,
				   Distrito,Provincia,Departamento,CodigoCredito,TipoCredito,SubTipoCredito,ProductoCrediticio,
				   SubProductoCrediticio,MontoDesembolso,SaldoCapital,Moneda,TipoCambio,MontoDesembolsoenSoles,
				   SaldoCapitalenSoles,TEM,NumeroCuotas,DiasAtraso,EstadoCredito,cDesModCre,FechaDesembolsoCredito,
				   Anio,NombreMes,Oficina,Zona,CodAsesorActual,NombreAsesorActual,CantEnt,CalifSBS,CuotaPag,CuotaPend,
				   Porcentaje,DiasPromAtr,CuotaMax
			from #tab04
