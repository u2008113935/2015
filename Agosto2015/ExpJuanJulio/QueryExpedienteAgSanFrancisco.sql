		...
		select * into #t01 from (
			...
			where cCodCtaCre COLLATE SQL_Latin1_General_CP1_CI_AS	
					in  (select Credito from ExpAgSanFrancisco )
			--order by cCodCtaCre
			) as tmp
		...
		*/										
		...
		Declare @codcred char(18), @codcli char(12)
			...
			Declare cExp CURSOR FOR	
				select credito from ExpAgSanFrancisco (NOLOCK) 	
		...
		---------------------------------------------------------------
			...
			Select * from ExpAgSanFrancisco (NOLOCK) 

		--04 llenando el codigo expediente en ExpAgSanFrancisco
		...
		Declare @codclie char(12), @codexp char(9)
			...
			Declare cExp01 CURSOR FOR	
				select CodigoCliente from ExpAgSanFrancisco (NOLOCK) 	
