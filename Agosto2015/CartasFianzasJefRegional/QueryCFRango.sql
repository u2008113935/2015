	 ...
	 select * into #tab01 from (
		SELECT 
			...
			FROM KPYMCRECARFIA FIA (NOLOCK)				 
				...
				inner JOIN CMACHYOCLI_201506.DBO.CLIMClienteS CLITIT (NOLOCK) 						
					ON (CLITIT.CCODCLIENTE = GEN.CCODCLIENTE)
				--LEFT JOIN [hyo00402].CMACHYOCLI_MANIANA.DBO.CLIDExpediente EXPE (NOLOCK) 
				LEFT JOIN CMACHYOCLI_201506.DBO.CLIDExpediente EXPE (NOLOCK) 
				...
				inner JOIN CMACHYOCLI_201506.DBO.CLIMClienteS CLIFAV (NOLOCK) 
