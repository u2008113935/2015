	...
	/*
	Carta Fianza - Mirellita.

	Solicito información de la Carta Fianza N° 107-051-0118-2011-CMACHYO: 
		fecha de otorgamiento, 
		si ha tenido ampliación- renovación (fecha de ampliación- renovación) 
		agencia 
		código cliente 
		usuario.
	...
	*/
	...
	DECLARE @nTipCambio MONEY,	@dFecTipCam DATE					
		set @nTipCambio = (
			...
			FROM GENTTipCambio
			WHERE left(cast(dFecTipCam as date),10) ='2015-08-31'				
				)					
	 ...
	 select * into #tab01 from (
		SELECT 
			...
			FROM KPYMCRECARFIA FIA (NOLOCK)				 
				...
				inner JOIN CMACHYOCLI_201507.DBO.CLIMClienteS CLITIT (NOLOCK) 										
					ON (CLITIT.CCODCLIENTE = GEN.CCODCLIENTE)
				LEFT JOIN CMACHYOCLI_201507.DBO.CLIDExpediente EXPE (NOLOCK) 				
					ON (EXPE.CCODCLIENT = CLITIT.CCODCLIENTE AND EXPE.CTIPEXPCLI = 'K'
						AND EXPE.LCONESTADO = 1) 
				inner JOIN CMACHYOCLI_201507.DBO.CLIMClienteS CLIFAV (NOLOCK) 				
			...
			WHERE --FIA.CESTCARFIA = 'K' -- in ('K','G')
				...
				--and FIA.CCODCTACRE = '10705101182011'
				--and ZON.nCodZona = '2'		
				CLITIT.cCodCliente = '107015761056'	
				AND FIA.CCODCLIENTE = '107016727567'
			) as tmp
			...
			*/						
			...
			Select * from CMACHYOCLI_201507.DBO.CLIMClienteS (NOLOCK) 										
			Where cNomCliente like '%INV%FRITZ%'

			Select * from #tab01

			Select * from KPYDGasGenFia 
			where cCodCtaCre = '107051101000191664'
