	...
	*/
	...
	Select 		
		...
		WHERE CRE.cEstCreCon in ('F','H','G','I')					
			...
			and left(cast(CRE.dFecDesCre as date),10) <= '2015-08-31'						
		) as tmp
		...
		*/
		...
		------------------------------------------------------
			...
			FROM GENTTipCambio
			WHERE left(cast(dFecTipCam as date),7) >='2015-09'
				and left(cast(dFecTipCam as date),7) <='2015-09' 
		...
		*/
		...
		WHERE PREN.cCodEstKpr 
			...
			and left(cast(PREN.dFecCreKpr as date),10) >= '2015-08-01'
			AND left(cast(PREN.dFecCreKpr as date),10) <= '2015-08-31'			
