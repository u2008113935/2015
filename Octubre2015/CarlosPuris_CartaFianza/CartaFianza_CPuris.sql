	...
	*/
	...
	DECLARE @nTipCambio MONEY,	@dFecTipCam DATE					
		set @nTipCambio = (
			...
			FROM GENTTipCambio
			WHERE left(cast(dFecTipCam as date),10) ='2016-06-30'				
				)					
	 ...
	 select * into #tab01 from (
		SELECT 
			...
			FROM KPYMCRECARFIA FIA (NOLOCK)				 
				...
				inner JOIN CMACHYOCLI.DBO.CLIMClienteS CLITIT (NOLOCK) 										
					ON (CLITIT.CCODCLIENTE = GEN.CCODCLIENTE)
				LEFT JOIN CMACHYOCLI.DBO.CLIDExpediente EXPE (NOLOCK) 				
					ON (EXPE.CCODCLIENT = CLITIT.CCODCLIENTE AND EXPE.CTIPEXPCLI = 'K'
						AND EXPE.LCONESTADO = 1) 
				inner JOIN CMACHYOCLI.DBO.CLIMClienteS CLIFAV (NOLOCK) 				
			...
			WHERE FIA.CESTCARFIA = 'K' -- in ('K','G')
				...
				--and left(cast(FIA.dFecSolFia as date),10) >= '2014-07-01'
				--and left(cast(FIA.dFecSolFia as date),10) <= '2015-07-31'
			...
			Select * from KPYDGasGenFia Where cCodCtaCre = '107002101002844443'
			Select * from #tab01 where CodigoCredito = '107002101002844443'

			Select * FROM KPYMCRECARFIA FIA (NOLOCK)  where cCodCtaCre = '107002101002844443'	 
		
			Declare @cCodCred1  char(18), @monpaggas1 numeric(14,4), @fechapag1 date
			
			SET @cCodCred1 = '107002101002844443'

			set @monpaggas1 = ( 
							SELECT nMonPagGas
							FROM KPYDGasGenFia
							WHERE cCodCtaCre = @cCodCred1
								   AND cCodTipGas = '11'
								   AND cCondicPag = 'N'
								   AND cCodEstGas = 'P'
								   and dFecPagGas in (
					...
					Select @cCodCred1 , @monpaggas1 , @fechapag1 
...
SELECT *
FROM #tab01
WHERE EstadoCartaFianza = 'COBRO COMISION'

CodigoCredito
107011101005787963


SELECT CodigoCredito, cantidad = COUNT(CodigoCredito)
FROM #tab01
WHERE EstadoCartaFianza = 'COBRO COMISION'
GROUP BY CodigoCredito
ORDER BY COUNT(CodigoCredito) ASC


-- EstadoCartaFianza : CANCELADO  COBRO COMISION

SELECT cCodCtaCre, nMonPagGas = ISNULL(nMonPagGas,0),dFecPagGas
FROM KPYDGasGenFia
WHERE cCodCtaCre = '107011101005787963' -- @cCodCred
	AND cCodTipGas = '11'
	AND cCondicPag = 'N'
	AND cCodEstGas = 'P'
	AND dFecPagGas IN (SELECT max(dFecPagGas)
						FROM KPYDGasGenFia
			...
			Alter table #tab01
			Add Comision numeric(14,4), FechaPag date		
			
			----------------------------cursor comision-----------------------
					Declare @cCodCred char(18) , @monpaggas money, @fechapag date							
				
					Declare cGarantiasUnion CURSOR FOR
						Select CodigoCredito From #tab01 (NOLOCK)					

					OPEN cGarantiasUnion
					FETCH cGarantiasUnion into @cCodCred
					WHILE (@@FETCH_STATUS=0)
					BEGIN		
		
					  set @monpaggas = ( 
							SELECT isnull(nMonPagGas,0)
							FROM KPYDGasGenFia
							WHERE cCodCtaCre = @cCodCred
								   AND cCodTipGas = '11'
								   AND cCondicPag = 'N'
					...
					*/
					...
					Select
						ROW_NUMBER() 
						OVER(PARTITION BY AnioSolicitud
						ORDER BY FechaSolicitud) AS Secuencia, 	 
						AnioSolicitud,MesSolicitud
						,FechaProceso,FechaSolicitud,FechaInicioCarta,FechaCancelacionCarta
						,CodigoClienteSolicitante,NombreClienteSolicitante,ActividadEconomicaSolicitante
						,CodigoCIIUSolicitante,CodigoLineaCredito,DescripcionCIISolicitante,CodExpedienteCliente
						,CodigoCredito,NumeroCartaFianza,EstadoCartaFianza,ValorCartaFianza,MonedaCartaFianza
						,ValorEnSoles,PlazoCartaFianza,DescripcionModificaciónCarta,TipoCredito
						,SubTipoCredito,ProductoCrediticio,SubProductoCrediticio,CodigoClienteFavorable
						,NombreClienteFavorable,Oficina,Zona,CodAsesorActual,NombreAsesorActual,Comision,FechaPag
					
					INTO #tab02
					From #tab01					
					Where  (FechaPag is not null or comision is not null)


					SELECT EstadoCartaFianza, Comision =  SUM(Comision)
					FROM #tab02
					GROUP BY EstadoCartaFianza
		...
		/*
			...
			--04 AGRUPANDO LAS GARANTIAS
					...
					Select * into #tab03 from 
						(select top 1 
							cCodLinCre,DescripcionTipoGarantia,MontoGravamenSoles 
						from #tab02 ) as tmp
					Select * from #tab03
					Delete from #tab03
