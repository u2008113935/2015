-- DROP TABLE #tmpv01
		...
		Select 
			...
			,CRE.nMonSalVen AS 'SaldoCapitalVencido'
			,CRE.nTasintCom
		...
		alter table #tmpv01
		add CuotaPagadas int, CuotaPend int, CuotaProgramadas int, DiasAtraso int
		...
		--*********************************INDEXANDO****************************************************
		CREATE NONCLUSTERED INDEX #tmpv01_CODIGO_CREDITO_IXN ON #tmpv01(CODIGO_CREDITO)
		CREATE NONCLUSTERED INDEX #tmpv02_CODIGO_CREDITO_IXN ON #tmpv02(CODIGO_CREDITO)
		...
		Declare cCursor11 CURSOR FOR
			select CODIGO_CREDITO from #tmpv01 (nolock)
		OPEN cCursor11
		...
		BEGIN
			...
			set @CuotaPend = (select count(cNumCuoPla) from [KPYDPLANPAGCRE] 
									where cCodCtaCre = @cCodCtaCre and cCodEstCuo = 'E'
									and cCodEstPla ='E' )	
		...
		select * from #tmpv02 (NOLOCK) --(752 row(s) affected)
		select len(CODIGO_CLIENTE) from #tmpv02 (NOLOCK)

		---------------------------FUENTE DE INGRESO---------------------------------------
		Declare @ccodcli varchar(12), @tfi char(50)	, @fi char(100)			
					Declare cFuenteIngreso01 CURSOR FOR	
					select CODIGO_CLIENTE from #tmpv02 (NOLOCK)
					OPEN cFuenteIngreso01
						FETCH cFuenteIngreso01 into @ccodcli
						WHILE (@@FETCH_STATUS=0)
						BEGIN	
				...
				set @tfi =  (
						Select top 1	Case FI.cCodTipFin 
										 ...
										 END 								
						from CMACHYOCLI_201504.dbo.[CLIDFueIngreso] FI
								left jOIN  CMACHYOCLI_201504.dbo.[CLIMEMPEMPLEA] CLIE
				...
				set @fi =  (
						...
						FETCH cFuenteIngreso01 INTO @ccodcli
						END
						CLOSE cFuenteIngreso01
						DEALLOCATE cFuenteIngreso01
		----------------------------------------------------------